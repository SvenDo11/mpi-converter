...
export class MPIStatementProvider implements TreeDataProvider<MPIStatementItem> {
    getTreeItem(
    ...
    getChildren(element?: MPIStatementItem): ProviderResult<MPIStatementItem[]> {
        if (element) {
    ...
    getStatements(): MPIStatementItem[] {
        ...
        for( let pos of sendPos) {
            items.push(new MPIStatementItem("MPI_Send Line " + (pos.line+1), pos));
        }
        for( let pos of recvPos) {
            items.push(new MPIStatementItem("MPI_Recv Line " + (pos.line+1), pos));
        }
        items.sort((a, b) => {
